...
def _services_config() -> dict:
    return {
        "services": {
            "service-a": {"steps": [{"module": "components.pre_calibration", "class": "PreCalibrationComponent"}]},
            "service-b": {"steps": [{"module": "components.simulation", "class": "SimulationComponent"}]}
        }
    }
...
def test_scope_missing_is_valid(tmp_path: Path) -> None:
    ...
    _write_json(
        ...
        {
            "interceptors": {
                "logging": {"module": "interceptors.logging", "class": "LoggingInterceptor"}
            }
        },
    )
